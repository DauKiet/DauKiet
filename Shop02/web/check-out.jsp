        ...
        <body>
            ...
            <script>
                ...
                function updateQuantity(element, change) {
                    var quantity = element.parentElement.querySelector('.qty-text');
                    var newQuantity = parseInt(quantity.value) + change;
                    if (newQuantity < 1) {
                        newQuantity = 1;
                    }
                    quantity.value = newQuantity;
                    updateQuantityCartTotal();
                    let form = element.closest('form');
                    form.submit();
                }

                function updateQuantityCartTotal() {
                    let quantityInputs = document.querySelectorAll('.qty-text');
                    let total = 0;
                    quantityInputs.forEach(function (input) {
                        quantity = parseInt(input.value);
                        price = parseFloat(input.closest('tr').querySelector('.price span').innerText);
                        total += quantity * price;
                    });
                    document.querySelector('#subtotal').innerHTML = total + 'VND';
                    document.querySelector('#total').innerHTML = total + 'VND';
                }
            </script>
        </body>
